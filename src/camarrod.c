...
char *s21_strstr(const char *haystack, const char *needle) {  // check
  ...
  if (haystack != s21_NULL && needle != s21_NULL) {
    ...
    if (lenOne == lenTwo && lenOne == 0 && *haystack == '\0' &&
        *needle == '\0') {
      first = 1;
      shift = 1;
    }
...
void *s21_insert(const char *src, const char *str,
  ...
  if (str && src && start_index <= s21_strlen(src)) {
    ...
    for (s21_size_t i = 0; i < lenSrc + lenStr; i++) {
      if (start_index <= i && i < start_index + lenStr)
        answer[i] = str[i - start_index];
      else {
        if (i >= start_index + lenStr)
          answer[i] = src[i - lenStr];
        else
          answer[i] = src[i];
      }
    }
