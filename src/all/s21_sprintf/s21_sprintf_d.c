...
void ppts_width_align_right_i_d(char *num_s, char *str, int col_spaces,
  ...
  if (mys->fl != '0') {
    if (num_s[0] != '-' && num_s[0] != '+') {
      for (int i = 0; i < col_spaces; i++) strcat(str, " ");
      for (int i = 0; i < col_zeros; i++) strcat(str, "0");
      if (mys->block_zero == 0) strcat(str, num_s);
      ...
      if (num_s[0] == '-')
        ...
        strcat(str, "+");
      for (int i = 0; i < col_zeros; i++) strcat(str, "0");
      if (mys->block_zero == 0) strcat(str, &num_s[1]);
    }
...
void ppts_width_align_left_i_d(char *num_s, char *str, int col_spaces,
  ...
  if (num_s[0] != '-' && num_s[0] != '+') {
    for (int i = 0; i < col_zeros; i++)
      if (mys->block_zero == 0) strcat(str, "0");
    if (mys->block_zero == 0) strcat(str, num_s);
    for (int i = 0; i < col_spaces; i++) strcat(str, " ");
