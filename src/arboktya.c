...
// операционных систем mac и linux.
...
int s21_memcmp(const void* str1, const void* str2, s21_size_t n) {
  ...
  for (s21_size_t i = 0; i < n && result == 0; ++i) {
    if (*_str1 != *_str2) result = *_str1 - *_str2;
    ++_str1;
    ++_str2;
  }
  ...
  if (str != s21_NULL) {
    ...
    if (result != s21_NULL) {
      for (s21_size_t i = 0; i < s21_strlen(str); ++i) {
        result[i] = str[i];
        if (str[i] >= 'A' && str[i] <= 'Z') result[i] += 'a' - 'A';
      }
...
void* s21_trim(const char* src, const char* trim_chars) {
  ...
  if (src != s21_NULL) {
    if (s21_strlen(src) == 0)
      result = "";
    else if (trim_chars == s21_NULL || s21_strlen(trim_chars) == 0)
      result = s21_trim(src, " ");
    else {
